  ...
  return (
    <div className="w-full h-[calc(100vh-64px)] overflow-y-auto scroll-smooth pb-24 md:pb-8">
      <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop flex flex-col">
        ...
        {/* Main Layout Wrapper */}
        <div className="flex flex-col gap-8 md:gap-4 pt-6 md:pt-6 md:pb-4 min-h-[calc(100vh-64px)] justify-center">
          
          {/* Hero Section (Carousel Banner) */}
          <section className="relative w-full overflow-hidden rounded-2xl h-[300px] md:h-[35vh] md:min-h-[220px] md:max-h-[320px] shrink-0 flex flex-col justify-end">
            <img
            ...
            />
            <div className="relative z-10 w-full bg-gradient-to-t from-black/80 to-transparent p-6 md:p-6 md:pb-4">
          ...
          <section className="flex flex-col justify-center shrink-0">
            <h3 className="font-title-md text-title-md text-on-surface mb-stack-sm md:mb-2 text-center md:text-left">Our Best Sellers</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-4">
              {bestSellers.map((product) => (
                <div key={product.id} className="flex flex-col bg-surface-container-lowest rounded-xl shadow-[0_4px_24px_rgba(119,87,77,0.15)] overflow-hidden hover:shadow-lg transition-shadow">
                  <Link href="/product" className="relative aspect-[4/3] md:aspect-video overflow-hidden group block cursor-pointer">
                    <img
                      alt={product.name}
                    ...
                    />
                    {product.tag && (
                      <div className="absolute top-2 right-2 md:top-base md:right-base bg-tertiary text-on-tertiary px-2 py-1 rounded-full font-label-sm text-label-sm">{product.tag}</div>
                    )}
                  </Link>
                  <div className="p-gutter md:p-3 flex flex-col gap-base md:gap-2 flex-grow">
                    ...
                    <p className="font-body-md text-body-md md:font-body-sm md:text-body-sm text-on-surface-variant line-clamp-2 md:line-clamp-1">{product.desc}</p>
                    <div className="mt-auto pt-base md:pt-2 flex justify-between items-center">
          ...
          {/* Search Bar */}
          <section className="w-full max-w-2xl mx-auto pb-4 md:pb-2 shrink-0">
